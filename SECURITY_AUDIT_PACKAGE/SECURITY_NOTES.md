...
- `src/lib/passkey.ts` adds platform WebAuthn passkey management without reopening the JS master-string boundary: passkey metadata is stored as vault fields and recovery metadata is wrapped with the active vault encryption key. Browser credential-provider/proxy behavior and Android Credential Provider integration remain outside the current release boundary.
