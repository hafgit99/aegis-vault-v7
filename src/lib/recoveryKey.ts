...
export function computeSha256ChecksumByteSync(entropy: Uint8Array): number {
  let h0 = 0x6a09e667;
  ...
  for (let i = 0; i < 8; i++) {
    w[i] = ((entropy[i * 4] ?? 0) << 24) | ((entropy[i * 4 + 1] ?? 0) << 16) | ((entropy[i * 4 + 2] ?? 0) << 8) | (entropy[i * 4 + 3] ?? 0);
  }
  w[8] = 0x80000000;
...
export function generateRecoveryWords(): string[] {
  ...
  for (let byteIdx = 0; byteIdx < fullPayload.length && words.length < 24; byteIdx++) {
    value = (value << 8) | (fullPayload[byteIdx] ?? 0);
    bits += 8;
...
export function validateRecoveryWords(words: string[]): boolean {
  ...
  const actualChecksum = payload[32] ?? 0;
