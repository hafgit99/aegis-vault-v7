...
export function generateLegacyArgon2idKey(password: string, salt: string, m_cost = 1024, t_cost = 3, p_cost = 2, keyLen = 32): Uint8Array {
...
export function generateLegacyArgon2idHash(password: string, customSalt?: string): string {
  const salt = customSalt || btoa(String.fromCharCode(...window.crypto.getRandomValues(new Uint8Array(12))));
  const derived = generateLegacyArgon2idKey(password, salt, 1024, 3, 2, 32);
  const derivedBase64 = btoa(String.fromCharCode(...derived));
  return `$argon2id$v=19$m=1024,t=3,p=2$${salt}$${derivedBase64}`;
}

export function verifyLegacyArgon2idHash(password: string, expectedHash: string): boolean {
  try {
    ...
    const derived = generateLegacyArgon2idKey(password, salt, m_cost, t_cost, p_cost, 32);
    const derivedBase64 = btoa(String.fromCharCode(...derived));
...
export function decryptLegacyAes256Gcm(payload: EncryptedPayload, key: Uint8Array): string {
...
export function decryptLegacyDataWithPassword(envelopeJsonStr: string, password: string): string {
  let parsed: any;
  try {
  ...
  if (parsed.version === "1.1" || parsed.kdf === "Argon2id") {
    ...
    const aesKey = generateLegacyArgon2idKey(password, parsed.salt, 1024, 3, 2, 32);

    // Decrypt GCM
    return decryptLegacyAes256Gcm({
      iv: parsed.iv,
      tag: parsed.tag,
