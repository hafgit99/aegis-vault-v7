...
- `src/lib/passkey.ts` and the Settings passkey manager add platform WebAuthn passkey management without reopening the JS master-string boundary: users can create local WebAuthn credentials, authenticate an existing record, update last-used/sign-count metadata, and delete records. Passkey metadata is stored as vault fields and recovery metadata is wrapped with the active vault encryption key. Browser credential-provider/proxy behavior and Android Credential Provider integration remain outside the current release boundary.
