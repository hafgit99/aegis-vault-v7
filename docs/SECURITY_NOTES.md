...
- Android Autofill is registered as a native service and settings entry. It can detect login-like forms from field metadata, show an authenticated Aegis entry point, and expose a pending launch request to the WebView bridge, but it does not fill stored credentials until package/domain verification and user approval are implemented.
...
- Android Autofill currently exposes provider registration, settings activation, login-field detection, an authenticated launch entry point, and a pending-request bridge only. Credential fill responses remain blocked until the matching and approval model is implemented.
