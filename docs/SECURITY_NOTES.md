...
- Domain matching (eTLD+1) in the browser extension and the Rust IPC host uses the full Mozilla Public Suffix List (10k+ rules, baked snapshot with pinned SHA-256), implementing standard wildcard and exception rule semantics — curated-subset mismatches that could collapse unrelated sites under one registrable domain are eliminated (M6).
...
- `src/lib/otp.ts` supports the common RFC 6238 HMAC-SHA1 path plus SHA-256/SHA-512 algorithm variants and `otpauth://totp` URI parsing for issuer/account imports. Unsupported TOTP digit counts and period values now fail with explicit validation errors instead of displaying a misleading placeholder code.
- `src/lib/passkey.ts` and the Settings passkey manager add platform WebAuthn passkey management without reopening the JS master-string boundary: users can create local WebAuthn credentials with strict RP ID validation, authenticate an existing record, update last-used/sign-count metadata, and delete records. Passkey metadata is stored as vault fields and recovery metadata is wrapped with the active vault encryption key. Browser credential-provider/proxy behavior and Android Credential Provider integration remain outside the current release boundary.
