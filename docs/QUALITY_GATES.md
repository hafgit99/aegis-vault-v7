...
- wa-sqlite repository skeleton: added the first real `WaSqliteVaultStorageRepository` with schema hydration, Argon2id-backed master setup/verification, metadata-only reads after credential verification, no plaintext password-keyed cache, and fail-closed vault write surfaces until encrypted row writes are ready.
